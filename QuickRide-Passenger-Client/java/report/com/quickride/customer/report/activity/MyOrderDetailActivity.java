...
public class MyOrderDetailActivity extends MGestureSwitchPageActivity {
	...
	private void setupOrderDetail(final Map<String, Object> order) {
		...
		((TextView) findViewById(R.id.rent_car_status)).setText(getString(R.string.rent_car_status) + "："
				+ getResources().getStringArray(R.array.order_status)[orderStatus - 2]);

		((TextView) findViewById(R.id.order_car_time)).setText(getString(R.string.order_car_time) + "："
		...
		((TextView) findViewById(R.id.rent_cost)).setText(getString(R.string.rent_cost) + "：" + order.get("price")
				+ getString(R.string.yuan) + " （" + isPayString + "）");
		...
		if (null != paymentAmount) {
			...
			paymentAmountView.setText("应付金额" + "：" + paymentAmount + getString(R.string.yuan));
		...
		((TextView) findViewById(R.id.rent_mileage)).setText(getString(R.string.rent_mileage) + "："
				+ order.get("distance") + getString(R.string.km));

		((TextView) findViewById(R.id.get_on_car_time)).setText(getString(R.string.get_on_car_time) + "："
				+ dateUtil.getDateString(Long.valueOf((String) order.get("pickupTime"))));

		((TextView) findViewById(R.id.get_on_car_location)).setText(getString(R.string.start_place) + "："
				+ order.get("pickupAddress"));

		((TextView) findViewById(R.id.get_off_car_location)).setText(getString(R.string.end_place) + "："
				+ order.get("unloadAddress"));

		((TextView) findViewById(R.id.car_type)).setText(getString(R.string.car_type) + "：" + order.get("carModel"));

		if (null != order.get("licenseNo")) {
			((TextView) findViewById(R.id.car_license)).setText(getString(R.string.car_license) + "："
					+ order.get("licenseNo"));
		}

		if (null != order.get("driverAverageRate")) {
			((RatingBar) findViewById(R.id.driver_grade)).setRating(Float.valueOf((String) order
					.get("driverAverageRate")));

			findViewById(R.id.grade).setVisibility(View.VISIBLE);
		}

		if (null != order.get("driverNo")) {
			((TextView) findViewById(R.id.driver_id)).setText(getString(R.string.driver_id) + "："
					+ order.get("driverNo"));
		}

		if (null != order.get("driverName")) {
			((TextView) findViewById(R.id.driver_name)).setText(getString(R.string.driver_name) + "："
		...
		} else if (orderStatus == StatusCode.BOOKED_CAR || orderStatus == StatusCode.BOOK_CAR) {
			...
			if (null == payType || (1 == payType && 0 == isPay)) {
				...
				payButton.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						Intent intent = new Intent(MyOrderDetailActivity.this, ChosePaymentTypeActivity.class);
	...
	private void showReviewInfo(final Map<String, Object> order) {
		...
		if (null == grade || grade == 0) {
			...
			addCommentButton.setOnClickListener(new Button.OnClickListener() {
				@Override
				public void onClick(View v) {
					final View textEntryView = LayoutInflater.from(MyOrderDetailActivity.this).inflate(
							R.layout.review_dialog, null);

					reviewDialog = new AlertDialog.Builder(MyOrderDetailActivity.this).setIcon(R.drawable.ic_menu_more)
							.setTitle("添加评价").setView(textEntryView).create();

					Button addCommentCommit = (Button) textEntryView.findViewById(R.id.confirm);
					addCommentCommit.setOnClickListener(new Button.OnClickListener() {
						@Override
						public void onClick(View v) {
							EditText addCommentEdit = (EditText) textEntryView.findViewById(R.id.edit_review);

							final String commentValue = addCommentEdit.getEditableText().toString();
							// if (null == commentValue ||
							// commentValue.trim().length() == 0) {
							// Toast.makeText(getApplicationContext(), "评论不能为空",
							// Toast.LENGTH_SHORT).show();
							//
							// addCommentEdit.setError("评论不能为空");
							// } else {
							final ProgressDialog progressDialog = ProgressDialog.show(MyOrderDetailActivity.this, null,
									getString(R.string.loading), true, true);

							RatingBar ratingBar = (RatingBar) textEntryView.findViewById(R.id.edit_stars);

							final int grade = (int) ratingBar.getRating();
							...
							new EndpointClient(MyOrderDetailActivity.this) {
								...
								protected Map<String, Object> doInBackground(Void... arg0) {
									...
									rate.put("orderNo", (String) order.get("orderNo"));
									rate.put("rate", String.valueOf(grade));
								...
								protected void onEndpointClientPostExecute(Map<String, Object> result) {
									if (null == result) {
										...
										Toast.makeText(MyOrderDetailActivity.this, R.string.request_fail,
									...
									if (StatusCode.SUCCESS.equals((String) result.get("statusCode"))) {
										Toast.makeText(MyOrderDetailActivity.this, "评价成功", Toast.LENGTH_LONG).show();

										order.put("rate", grade);
										order.put("comment", commentValue);
										...
										Toast.makeText(MyOrderDetailActivity.this,
												getString(R.string.request_fail) + ":" + result.get("statusMessage"),
												Toast.LENGTH_LONG).show();
									}
								}
							}.execute();
					...
					Button addCommentCancel = (Button) textEntryView.findViewById(R.id.cancel);
					addCommentCancel.setOnClickListener(new Button.OnClickListener() {
						@Override
						public void onClick(View v) {
							reviewDialog.cancel();
						}
			...
			((RatingBar) findViewById(R.id.grade_rating_bar)).setRating((Integer) order.get("rate"));

			String comment = (String) order.get("comment");
			if (null != comment && comment.trim().length() > 0) {
				((TextView) findViewById(R.id.review)).setText(getString(R.string.review) + "：" + order.get("comment"));
				findViewById(R.id.review).setVisibility(View.VISIBLE);
			}

			((LinearLayout) findViewById(R.id.review_layout)).setVisibility(View.VISIBLE);
		}
	}
	...
	private void viewOrderDetail(final String orderNo) {
		...
		progressDialog.setOnCancelListener(new OnCancelListener() {
			...
			public void onCancel(DialogInterface dialog) {
				new AlertDialog.Builder(MyOrderDetailActivity.this).setIcon(R.drawable.alert)
						.setTitle(R.string.loading).setMessage(R.string.confirm_retry)
						.setPositiveButton(R.string.confirm, new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface dialog, int whichButton) {
								MyOrderDetailActivity.this.viewOrderDetail(orderNo);
							}
