 ...
 * 修改电话号码页面<br>
 * 完全拷贝的
 * {@link com.quickride.customer.security.activity.ModifyPhoneNumberActivity}
 * 待重构
 * 
 * @author eastseven
 * 
...
public class ModifyPhoneNumberActivity extends MGestureSwitchPageActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.main_modify_phone_number);
	...
	void initHeaderBar() {
		...
		this.headerTitle.setText(getString(R.string.main_modify_phone_number));
	...
	void initContent() {
		progressDialog = MProgressDialog.show(this, null, getString(R.string.loading), true, true, new OnCancelListener() {
			@Override
		...
		registerReceiver(countDownReceiver = new CountDownReceiver(getVerificationSmsButton), new IntentFilter(CountDownService.class.getName()));
		...
		new EndpointClient(this) {
			...
			protected void onEndpointClientPostExecute(Map<String, Object> result) {
				if (null == result) {
					Toast.makeText(ModifyPhoneNumberActivity.this, getString(R.string.loading_fail), Toast.LENGTH_SHORT).show();
				...
				if (success) {
					...
					Toast.makeText(ModifyPhoneNumberActivity.this, (String) result.get("message"), Toast.LENGTH_LONG).show();
		...
		confirm.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				progressDialog = ProgressDialog.show(ModifyPhoneNumberActivity.this, null, getString(R.string.updating), true, true);
				...
				new EndpointClient(ModifyPhoneNumberActivity.this) {
					...
					protected void onEndpointClientPostExecute(Map<String, Object> result) {
						...
						if (null == result) {
							...
							Toast.makeText(ModifyPhoneNumberActivity.this, R.string.modify_fail, Toast.LENGTH_LONG).show();
						...
						if (true == success) {
							Toast.makeText(ModifyPhoneNumberActivity.this, R.string.modify_success, Toast.LENGTH_LONG).show();
							...
							Toast.makeText(ModifyPhoneNumberActivity.this, getString(R.string.modify_fail) + result.get("message"), Toast.LENGTH_LONG).show();
						}
					}
				}.execute();
			}
	...
	private boolean validatePhoneNumber() {
		...
		if (null == verificationCodeEditText.getText().toString() || "".equals(verificationCodeEditText.getText().toString().trim())) {
